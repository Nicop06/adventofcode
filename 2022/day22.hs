...
nextTile grid (coord, dir) = let (x, y) = wrapGrid grid $ nextCoord grid dir coord in case (grid ! ix2 x y) of
    None -> nextTile grid ((x, y), dir)
    otherwise -> ((x, y), dir)

move :: Int -> NextTileFunc -> Grid -> Player -> Player
move 0 _ _ p = p
move steps nextTileFunc grid player = let ((x, y), dir') = nextTileFunc grid player in case (grid ! ix2 x y) of
    Wall -> player
    otherwise -> move (steps - 1) nextTileFunc grid ((x, y), dir')
...
nextTileCube grid p@((x, y), dir) = let (x', y') = nextCoord grid dir (x, y) in
    if isOnTile grid (x', y')
    then ((x', y'), dir)
    else wrapCube
        where wrapCube
